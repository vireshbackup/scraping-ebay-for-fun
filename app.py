...
class Site(db.Model):
    ...
    activitys = db.relationship('Activity', backref='tracking_site', lazy='select')
...
class Activity(db.Model):
    ...
    def __repr__(self):
        return '<Activity %r - %r>' % (self.url, self.date)
...
def add_activity():
    ...
    if form.validate_on_submit():
        ...
        activity.site_id = form.site.data.id
        db.session.add(activity)
        db.session.commit()
        flash("Added activity for site " + form.site.data.base_url)
        return redirect(url_for("index"))
    return render_template("validation_error.html", form=form)
...
def _make_link(site_id):
    url = url_for("view_site_activitys", site_id=site_id)
    return _LINK.format(url=url, name=site_id)


@app.route("/site/<int:site_id>")
def view_site_activitys(site_id=None):
    site = Site.query.get_or_404(site_id)
    query = Activity.query.filter(Activity.site_id == site_id)
    data = query_to_list(query)
    title = "activitys for " + site.base_url
